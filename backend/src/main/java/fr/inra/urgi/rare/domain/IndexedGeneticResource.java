...
/**
 * Interface common to all indexed genetic resources that can be indexed and searched by this application.
 * One implementation exists for each app: one for Rare, one for WheatIS, etc.
 *
 * It contains all the fields of a GeneticResource, and additional fields used uniquely for indexing
 * and which thus make it possible or easier to implement completion suggestions.
 * @author JB Nizet
 */
public interface IndexedGeneticResource<R extends GeneticResource> {
    /**
     * Returns the wrapped genetic resource
     */
    R getGeneticResource();
